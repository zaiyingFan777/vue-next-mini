...
export function h(type: any, propsOrChildren?: any, children?: any): VNode {
  ...
  if (l === 2) {
    // 如果 第二个参数是对象，但不是数组。则第二个参数只有两种可能性：
    // * 1. VNode 2.普通的 props
    ...
    // * !isObject(propsOrChildren) || isArray(propsOrChildren)
    // 非普通对象（如 string/number）或数组 → 视为 children
    // （普通对象且非数组已在上方处理为 props）
    else {
      return createVNode(type, null, propsOrChildren)
    }
  }
  // 如果用户传递了三个或以上的参数，那么证明第二个参数一定代表了 props
    ...
    if (l > 3) {
      // * Array.prototype.slice.call([1,2,3,4,5], 2) => [3, 4, 5]
